...
public class Estudiantes2 {
    public static void main(String[] args) {
        //datos de conexion a la base de datos (ajustar valores segun a configuracion individual)
        ...
        try{
            //establecer la configuracion a la base de datos
            ...
            while(true){
                ...
                scan.nextLine(); //consumir la nueva linea
                ...
                switch(opcion){
                    case 1:
                        ...
                        break;
                    case 2:
                        agregarEstudiante(conexion, scan);
                        break;
                    case 3:
                        editarEstudiante(conexion, scan);
                        break;
                    case 4:
                        eliminarEstudiante(conexion, scan);
                        break;
                    case 5:
    ...
    private static void mostrarEstudiantes(Connection conexion) throws SQLException{
        //Crea una declaracion SQL para ejecutar una consulta de seleccion
        Statement statement = conexion.createStatement();

        //define la consulta SQL para obtener todos los estudiantes
        String consulta = "SELECT * FROM estudiantes";

        //ejecuta la consulta SQL y alamcena los resultados en un ResultSet;
        ...
        //itera atravez de los resultados y muestra los datos de cada estudiantes en forma de tabla
        ...
        while(resultado.next()){
            ...
            //imprime datos del estudiante con tabulaciones
            System.out.println(id + "\t" + nombre + "\t" + apellido + "\t" + legajo + "\t" + dni + "\t" +
                    fechaNacimiento + "\t" + direccion + "\t" + telefono + "\t" + email);

        }
        //cierre el ResultSet y el Satement para liberar recursos
        resultado.close();
        statement.close();
    }
    ...
    private static void agregarEstudiante(Connection conexion, Scanner scan) throws SQLException{
        //solicita al ususario que ingrese los datos del estudiante
        ...
        //consulta SQL para insertar un nuevo estudiante a la base de datos
        //Utiliza signos de interrogacion como marcadores de posicion para los valores.

        String consulta = "INSERT INTO  estudiantes (nombre, apellido, legajo, fecha_nacimiento, direccion, telefono, email)" +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

        //crea un PeparedStatement para ejecutar la consulta SQL con valores reales
        ...
        //verifica si la insercion fue exitosa y muestra un mensaje apropiado
        if(filasAfectadas > 0){
        ...
        //cierra  el PreparesStatement para liberar recursos
        preparedStatement.close();
    }

    //funcion para editar un estudiante
    //peprmite editar un estudiante existente en la base de datos

    private static void editarEstudiante(Connection conexion, Scanner scanner)throws SQLException{
        //solicita al ususario que ingrese la id del estudiante que desea editar
        System.out.println("ID del estudiante a editar: ");
        int id = scanner.nextInt();
        scanner.nextLine(); //consumir la nueva linea despues de leer el entero

        //verificar si el estudiante con el id espicificado existe en la base de datos
        ...
        preparedStatementExistencia.setInt(1,id); //establece el valor del marcador de posicion
        ResultSet resultadoExistencia =  preparedStatementExistencia.executeQuery();

        //si no se encuentra ningun estudiante con el id solicitda, se muestra un mensaje y sale de la funcion
        ...
        //solicita el usuario ingresar los nuevos datos del estudiante
        ...
        //consulta de SQL para actualizar los datos del estudiante  en la base de datos

        String consulta = "UPDATE estudiantes SET nombre = ?, apellido = ?, legajo = ?, dni = ?, " +
                "fecha_nacimiento = ?, direccion = ?, telefono = ?, email = ? WHERE id = ?";

        //se crea un PreparedStatement para ejecutar la consulta SQL con valores reales
        PreparedStatement preparedStatement = conexion.prepareStatement(consulta);
        ...
        //ejecuta la consulta SQL y obtiene el numero de filas afectadas
        int filasAfectadas = preparedStatement.executeUpdate();

        //verifica si la edicion fue exitosa y muestra un mensaje apropiado
        if (filasAfectadas > 0) {
        ...
        //esta funcion permite  eliminar un estudiante de la base de datos
